...
KLEIN_FOUND="$(find "$MODELS_DIR" -maxdepth 1 -type f -iname "*klein*" | head -1 || true)"
...
if ! find "$CHECKPOINTS_DIR" -maxdepth 1 -type f -iname "*22b-distilled*.safetensors" | grep -q .; then
  echo "❌ No se pudo descargar el checkpoint base de LTX-2.3"
  exit 1
fi
if [ ! -f "$TEXT_ENCODERS_DIR/gemma_3_12B_it_fp4_mixed.safetensors" ]; then
  echo "❌ Falta Gemma 3 text encoder para LTX-2.3"
  exit 1
fi
if ! find "$LORAS_DIR" -maxdepth 1 -type f -iname "*distilled-lora*.safetensors" | grep -q .; then
  echo "❌ Falta distilled LoRA de LTX-2.3"
  exit 1
fi
if ! find "$LATENT_UPSCALE_DIR" -maxdepth 1 -type f -iname "*upscaler-x2*.safetensors" | grep -q .; then
  echo "❌ Falta upscaler-x2 de LTX-2.3"
  exit 1
fi
...
apt-get install -y -qq ffmpeg libgl1 libglib2.0-0 2>/dev/null || true
